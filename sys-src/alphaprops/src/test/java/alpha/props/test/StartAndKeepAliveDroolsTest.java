...
/**
 * The Class StartAndKeepAliveDroolsTest.
 */
@Test(enabled = false)
public class StartAndKeepAliveDroolsTest implements Observer {
	...
	public void setUp() {
		...
		final InputStream fis = this.getClass().getClassLoader()
				.getResourceAsStream("alphaconfig2.properties");
		...
		try {
			StartAndKeepAliveDroolsTest.LOGGER.finer("Initialize model...");
			this.alphaPropsFacade.initializeModel("./Desktop/myDoc");
		} catch (final VerVarStoreException e) {
			StartAndKeepAliveDroolsTest.LOGGER.severe("" + e);
		} catch (final Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		StartAndKeepAliveDroolsTest.LOGGER
				.info("Create AlphaDoc artificially...");
		this.doc = this.boostrapAlphaDoc();

		StartAndKeepAliveDroolsTest.LOGGER.info("Initialize config...");
		// alphaPropsFacade.initializeConfig(properties);
		this.alphaPropsFacade.initializeConfig("23456");
		...
		try {
			while (true) {
				...
				StartAndKeepAliveDroolsTest.LOGGER.info("Time: " + new Date());
			}
	 ...
	 */
	private AlphaDoc boostrapAlphaDoc() {
